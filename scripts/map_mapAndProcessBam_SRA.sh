...
# READ GROUPS --------------------------------------------------------------------
#read groups are weird from the SRA.  Add them semi-manually here.
for SAMPLE in   Sm.BR_0447.1 Sm.BR_1278.1 Sm.BR_2039.1; do
    ...
    $SINGULARITY gatk AddOrReplaceReadGroups \
        --INPUT=$IN_BAM \
        --OUTPUT=$OUT_BAM \
        --RGID=$CELL.$LANE \
        --RGLB=library1 \
...
for SAMPLE in   ERR119623 SRR433865 ERR103048; do
    IN_FASTQ=$RAW_READS_DIR/$SAMPLE/$SAMPLE"_R1.fastq.gz"
    IN_BAM=$MAP_DIR/$SAMPLE"_merged.bam"
    OUT_BAM=$MAP_DIR/$SAMPLE"_merged_RGs.bam"

    CELL=$(echo $HEADER_LINE | cut -f1 -d":" | cut -f2 -d" ")
    LANE=$(echo $HEADER_LINE | cut -f2 -d":")
    INDEX=UNK
...
for SAMPLE in   ERR037800 ERR084970 ERR119622; do
    IN_FASTQ=$RAW_READS_DIR/$SAMPLE/$SAMPLE"_R1.fastq.gz"
    IN_BAM=$MAP_DIR/$SAMPLE"_merged.bam"
    OUT_BAM=$MAP_DIR/$SAMPLE"_merged_RGs.bam"
