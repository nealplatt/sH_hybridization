...
for vcf_entry in vcf_in:
    ...
    if not vcf_entry.startswith("#"):
        ...
        mat_gts=[[i.split(':')[0] for i in gt][2], [i.split(':')[0] for i in gt][7], [i.split(':')[0] for i in gt][8]]
        ...
        for allele in bov_alleles:
            if allele not in cur_alleles or allele not in out_alleles and allele is not '.':
                snps_out.write(id + "\tbov\t" + allele + "\n")
                #snps_out.write(id + "bov\t" + allele + "\t" + cur_alleles+ "\t" + out_alleles +'\n')
        for allele in cur_alleles:
            if allele not in bov_alleles or allele not in out_alleles and allele is not '.':
